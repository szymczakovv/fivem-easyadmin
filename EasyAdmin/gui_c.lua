...
Citizen.CreateThread(function()
	...
	mainMenu = NativeUI.CreateMenu("EasyAdmin", "www.szymczakovv.pl", menuOrientation, 0)
...
RegisterCommand('easyadmin', function(source, args, raw) coolfunction() end)
...
function daojosdinpatpemata()
    ...
    if
     ...
     then
        ...
        av('Odwrócono pojazd!', false)
    else
        av("Nie jesteś w pojeździe!", true)
    end
end
...
function GenerateMenu() -- this is a big ass function
	...
	if permissions.slap then
		...
		ogolne:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerEvent('OpenLSMenu') -- trigger z lscutom lub innego skryptu twojego od tego
		end
	end
	...
	if permissions.slap then
		local thisItem = NativeUI.CreateItem("Restart Serwera (Kick Players)", "")
		ogolne:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			TriggerServerEvent('kickAllPlayer2')
			TriggerEvent('esx:showNotification', 'Kick all - Restart serwera!')
		end
	end
	

	if permissions.slap then
		local thisItem = NativeUI.CreateItem("Debug Plate (Dodaj Auto)", "")
		ogolne:AddItem(thisItem)
		thisItem.Activated = function(ParentMenu,SelectedItem)
			local vehicle = GetVehiclePedIsUsing(PlayerPedId())
			local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)
			TriggerServerEvent("esx_vehicleshop:setVehicleOwned", vehicleProps)
			TriggerEvent('esx:showNotification', 'Debug plate')
		end
	end
	...
	function tpm()
				...
				if DoesBlipExist(WaypointHandle) then
					...
					av("Przeteleportowano.", true)
				else
					av("Zaznacz na mapie teleport.", true)
				end
	end
	...
	for i,thePlayer in ipairs(players) do
		...
		if permissions.ban then
			local thisItem = NativeUI.CreateItem("Daj Revive", "")
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
	...
	while true do
		...
		if drawInfo then
			...
			if IsControlJustPressed(0,103) then
				...
				TriggerEvent('esx:shoNotification','Prestałeś obserować użytkownika')
			end
