...
AddEventHandler("logMenuAdmin", function(option)
	...
	SendWebhookMessageMenuStaff(webhook,"**ADMIN MENU** \n```diff\nGracz: "..name.."\nID: "..source.." \nWykonana Akcja: "..option.."\n+ Data: " .. date.day .. "." .. date.month .. "." .. date.year .. " - " .. date.hour .. ":" .. date.min .. ":" .. date.sec .. "\n[ID ZGŁOSZENIA #".. platenum .."].```")
...
AddEventHandler("ReviveAll", function()
	...
	SendWebhookMessageMenuStaff(webhook,"**Admin ożywił cały serwer** \n```diff\nGracz: "..name.."\nID: "..source.." \n[ID ZGŁOSZENIA #".. platenum .."].```")
	...
	--[[RegisterCommand("calladmin", function(source, args, rawCommand)
		...
		if GetConvar("ea_enableReportCommand", "false") == "true" then
			...
			for i,_ in pairs(OnlineAdmins) do 
				...
				TriggerClientEvent('chatMessage', i, "^3!!EasyAdmin Report System!!^7\n"..string.format(string.gsub(GetLocalisedText("playercalledforadmin"), "```", ""), getName(source), source, reason))
